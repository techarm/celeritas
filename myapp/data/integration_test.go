...
func TestUser_GetByEmail(t *testing.T) {
	_, err := models.Users.GetByEmail("me@here.com")
	if err != nil {
		t.Errorf("failed to get user: %v", err)
	}
}
...
func TestUser_PasswordMatches(t *testing.T) {
	...
	if err != nil {
		t.Error("error check match:", err)
	}

	if !matches {
		t.Error("password doest match when it should")
	}
	...
	if matches {
		t.Error("password doest match when it should not")
	}
}

func TestUser_ResetPassword(t *testing.T) {
	err := models.Users.ResetPassword(1, "net_password")
	if err != nil {
		t.Error("error reseting password", err)
	}

	err = models.Users.ResetPassword(2, "net_password")
	if err == nil {
		t.Error("dit not get an error when trying to reset password for non-existen user")
	}
}
...
func TestUser_Delete(t *testing.T) {
	...
	if err == nil {
		t.Error("retrieved user who was supposed to be deelte")
	}
}
...
func TestToken_GenerateToken(t *testing.T) {
	...
	if err != nil {
		t.Error("error genertating token", err)
	}
}
